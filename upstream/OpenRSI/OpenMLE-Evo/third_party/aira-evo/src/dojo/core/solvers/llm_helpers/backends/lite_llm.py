...
class LiteLLMClient:
    ...
    def _query_client(
        ...
        usage_stats["effective_request_parameters"] = {k: (str(v) if isinstance(v, httpx.Timeout) else v) for k,v in filtered_kwargs.items() if k != "api_key"}
        if completion is not None and not use_stream:
            usage_stats["raw_response"] = completion.to_dict()
        ...
        usage_stats["token_usage_source"] = "provider" if "prompt_tokens" in usage_stats and "completion_tokens" in usage_stats else "partly_or_fully_estimated"
